...
const { processor, ram, hdd, os, software } = testData;
test(`Test build your computer when ordering 1 type product: CPU ${processor}, RAM: ${ram}, HDD: ${hdd}, OS: ${os}, Software: ${software}`, async ({ page }) => {
  ...
  await orderComputerFlow.openHomePageAndGoToSpecificProduct(testData);
  await orderComputerFlow.buildOwnComputerAndAddToCart(testData);
